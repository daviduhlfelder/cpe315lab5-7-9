...
static int counter(unsigned short intake) {
  int c = 0;
  unsigned int i;
  i = 0;
  while (i < 8) {
...
void execute() {
  ...
  case MISC:
    ...
    case MISC_PUSH:
      // need to implement
      count = counter(misc.instr.push.reg_list) + misc.instr.push.m;
      addr = SP - 4 * count;
      i = 0;
      while (i > 8) {
    ...
    case MISC_POP:
      // need to implement
      count = counter(misc.instr.pop.reg_list) + misc.instr.pop.m;
      addr = SP;
      i = 0;
      while (i > 8) {
      ...
      rf.write(SP_REG, SP + count * 4);
      stats.numMemReads += count;
      stats.numRegReads++;
      stats.numRegWrites += count + 1;
      break;
    case MISC_SUB:
      // functionally complete, needs stats
  ...
  case COND:
    ...
    if (checkCondition(cond.instr.b.cond)) {
      ...
      stats.numRegWrites++;
      stats.numRegReads++;
      if (2 * signExtend8to32ui(cond.instr.b.imm) > 0) {
  ...
  case LDM:
    decode(ldm);
    // need to implement
    count = counter(ldm.instr.ldm.reg_list);
    ...
    rf.write(ldm.instr.ldm.rn, rf[ldm.instr.ldm.rn] + (count * 4));
    stats.numMemReads += count;
    stats.numRegWrites += count + 1;
    stats.numRegReads++;
    break;
  case STM:
    decode(stm);
    // need to implement
    count = counter(stm.instr.stm.reg_list);
    ...
    rf.write(stm.instr.stm.rn, rf[stm.instr.stm.rn] + count * 4);
    stats.numMemWrites += count;
    stats.numRegWrites++;
    stats.numRegReads++;
    break;
  case LDRL:
